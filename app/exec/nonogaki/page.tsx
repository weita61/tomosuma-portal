...
const EXEC = { name: '野々垣 友祐', dept: 'テクノロジー部', color: '#185FA5', sec: '森川 あおい' }

export default function NonogakiPage() {
  const [messages, setMessages] = useState([
    { from: 'sec', text: '野々垣さん、おはようございます！本日のテクノロジー部MTGは19:00からです。' },
  ])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: 'sans-serif' }}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', gap: '10px', background: '#fff' }}>
        <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: EXEC.color, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: '600' }}>野</div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px', background: '#fafafa' }}>
        {messages.map((msg, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: msg.from === 'exec' ? 'flex-end' : 'flex-start', gap: '8px', alignItems: 'flex-end' }}>
            {msg.from === 'sec' && (
              <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#993556', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', flexShrink: 0 }}>森</div>
            )}
      ...
      <div style={{ padding: '12px 16px', borderTop: '1px solid #eee', display: 'flex', gap: '8px', background: '#fff' }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMsg()} placeholder="森川に送る..." style={{ flex: 1, padding: '10px 14px', borderRadius: '20px', border: '1px solid #ddd', fontSize: '12px', outline: 'none', background: '#f5f5f5' }} />
